...
import ke.co.eshuttle.eshuttle.config.KafkaProperties;
...
class EshuttleKafkaResourceIT {
    ...
    void setup() {
        ...
        EshuttleKafkaResource kafkaResource = new EshuttleKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/eshuttle-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/eshuttle-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
